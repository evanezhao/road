<template>
	<view class="content uni-flex uni-column" style="width: 100%; height: 98%; overflow-y: auto;">
		...
		<navigator v-show="!!orders" v-for="order in orders" :key="order.id" class="uni-margin-wrap uni-flex uni-row uni-card-shadow"
		...
		<navigator v-if="user.rule === 1 || !user.rule" url="/pages/customer/create" hover-class="navigator-hover" class="uni-margin-wrap uni-flex uni-row uni-center">
	...
	export default {
		...
		data() {
			return {
				...
				dateTime: '',
				viewUrlList: {
					"3": '/pages/servicer/view?id=',
					"2": '/pages/driver/view?id=',
					"1": '/pages/customer/view?id='
				},
				weather: {
					province: '--', //省
					city: '--', //城市
			...
			wx.getSetting({
				success(res) {
					...
					if (res.authSetting['scope.userLocation']) {
						...
						self.openLocAuto().then(function(isAuth) {
							if (isAuth) {
								self.getWeather(amapPlugin);
							}
						});
		...
		methods: {
			...
			...mapActions(['openLocAuto', 'setLocAuto'])
		}
	}
</script>
